component extends="wheels.Test"  hint="Unit Tests" {
	...
	function Test_getRecurringDates_daily(){
		r=c.getRecurringDates("FREQ=DAILY", seed, from, to);
		assert("arraylen(r) EQ 14");
	}
	function Test_getRecurringDates_weekly(){
		to=createDate(2017, 04, 15);
		r=c.getRecurringDates("FREQ=WEEKLY", seed, from, to);
